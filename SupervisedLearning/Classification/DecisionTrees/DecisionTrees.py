...
def implementation():
    ...
    plt.figure(figsize=(15,15))
    tree.plot_tree(model, feature_names=X.columns, class_names=[str(c) for c in model.classes_], filled=True)
    plt.show()

    # 2) Now train & plot on a small sample if you want a “zoomed‐in” tree
    sampleData = df2.sample(50, random_state=42)   # pick 50 random rows
